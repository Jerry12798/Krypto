...
async def create_ytdl_client(formating=None):
...
class MP3(commands.Cog, name="MP3", description="Music Commands"):
	...
	async def queued_error(self, ctx, error):
		if isinstance(error, commands.CommandInvokeError):
			embed = discord.Embed(title="__**Queue List Error**__", description=f"There is No Songs in {ctx.guild}'s Queue.", timestamp=datetime.now(), color=0xff0000)
			embed.set_footer(text=f"{ctx.author}", icon_url=ctx.author.display_avatar.replace(format="png", static_format="png"))
			await ctx.send(embed=embed)
		await ctx.message.delete()
	...
	async def join(self, ctx, *, channel:discord.VoiceChannel=None):
		if channel is None:
			...
			return
		if ctx.voice_client is not None:
			embed_2 = discord.Embed(title="__**Music Error**__", description=f"{self.bot.user.mention} is already Connected to **{ctx.voice_client.channel}**.", timestamp=datetime.now(), color=0xac5ece)
			embed_2.set_footer(text=f"{ctx.author}", icon_url=ctx.author.display_avatar.replace(format="png", static_format="png"))
			await ctx.send(embed=embed_2)
			await ctx.message.delete()
			return
	...
	async def pause(self, ctx):
		if ctx.voice_client is None:
			...
			await ctx.send(embed=embed)
			return
		embed_2 = discord.Embed(title="__**Music Paused**__", description=f"{self.bot.user.mention} has `Paused` the Music in **{ctx.voice_client.channel.name}**.", timestamp=datetime.now(), color=0xac5ece)
		embed_2.set_footer(text=f"{ctx.author}", icon_url=ctx.author.display_avatar.replace(format="png", static_format="png"))
	...
	async def resume(self, ctx):
		if ctx.voice_client is None:
			...
			await ctx.send(embed=embed)
			return
		embed_2 = discord.Embed(title="__**Music Resumed**__", description=f"{self.bot.user.mention} has `Resumed` the Music in **{ctx.voice_client.channel.name}**.", timestamp=datetime.now(), color=0xac5ece)
		embed_2.set_footer(text=f"{ctx.author}", icon_url=ctx.author.display_avatar.replace(format="png", static_format="png"))
	...
	async def skip(self, ctx):
		if ctx.voice_client is not None:
			embed = discord.Embed(title="__**Song Skipped**__", description=f"{self.bot.user.mention} has `Skipped` the Music in **{ctx.voice_client.channel.name}**.", timestamp=datetime.now(), color=0xac5ece)
			embed.set_footer(text=f"{ctx.author}", icon_url=ctx.author.display_avatar.replace(format="png", static_format="png"))
	...
	@commands.command() # Download Song Command
	async def mp3(self, ctx, *, song:str=None):
		if song is None:
			embed = discord.Embed(title="__**Music Error**__", description=f"You must Name a Song to Download.\n`{self.bot.prefix}mp3 <Song Name>`", timestamp=datetime.now(), color=0xff0000)
			embed.set_footer(text=f"{ctx.author}", icon_url=ctx.author.display_avatar.replace(format="png", static_format="png"))
			await ctx.send(embed=embed)
			return
