	...
	def __init__(self, bot):
		...
		async def ggwebhook():
			if request.headers.get('authorization') == TOPGG_Secret:
				...
				embed = discord.Embed(title=f"Upvote on Top.gg", url=TOPGG_Vote_Link, description=f"{member.mention} has Successfully Upvoted {self.bot.user.mention}", timestamp=datetime.utcnow(), color=0xac5ece)
				embed.set_author(name=f"{member}", icon_url=str(member.avatar_url_as(format=None, static_format="png")))
		...
		async def dbwebhook():
			if request.headers.get('authorization') == DBL_Secret:
				...
				log = self.bot.get_channel(self.bot.vote_log)
				member = self.bot.get_user(user)
				embed = discord.Embed(title=f"Upvote on Discord Bot List", url=DBL_Vote_Link, description=f"{member.mention} has Successfully Upvoted {self.bot.user.mention}", timestamp=datetime.utcnow(), color=0xac5ece)
				embed.set_author(name=f"{member}", icon_url=str(member.avatar_url_as(format=None, static_format="png")))
				await log.send(embed=embed)
